        ...
        if (mysqli_num_rows($resultComments) > 0) {
            ...
            while ($rowComment = mysqli_fetch_assoc($resultComments)) {
                if ($rowComment['status'] == "approved") {
                    ...
                    // Закрываем div для approved комментариев
                    echo '</div>';
                    echo '</div>';
                }
                    ...
                    if (!empty($_SESSION['auth'])) {
                        if ($rowComment['User_ID'] == $_SESSION['User_ID']) {
                            ...
                            echo '<div class="d-flex">
                                <div class="d-flex gap-2" > <div  class="btn  " style="color: white; height: max-content;  border: none; font-size: 13px !important; background-color: var(--ligth-blue) !important;"> ' . $rowComment['status'] . ' </div>
                                <a href="delete_comment.php?id=' . $rowComment['Comment_ID'] . '" class="btn btn-danger">Удалить</a>
                            </div>
                            </div>';
                            // Закрываем div для processing комментариев
                            echo '</div>';
                            echo '</div>';
                        }
                    }
            ...
            echo '</div>'; // Закрываем div для списка комментариев
